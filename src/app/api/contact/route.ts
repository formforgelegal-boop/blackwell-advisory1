...
const schema = z.object({
  ...
  contractors: z.string().trim().min(1),
  commissionBonuses: z.string().trim().min(1),
  restrictiveCovenants: z.string().trim().min(1),
  policiesIncorporated: z.string().trim().min(1),
  ...
  try {
    const lead = await withLeadMetadata({
      ...
      answers: {
        ...
        contractors: parsed.data.contractors,
        commissionBonuses: parsed.data.commissionBonuses,
        restrictiveCovenants: parsed.data.restrictiveCovenants,
        policiesIncorporated: parsed.data.policiesIncorporated,
